...
function getFreeMarkerConfig( loaderContext, options ) {
   ...
   const fmLoader = java.newProxy(
      TEMPLATE_LOADER,
      getTemplateLoaderImplementation( loaderContext, options.templateRootDirectory )
   );
...
function getTemplateLoaderImplementation( loaderContext, templateRootDirectory ) {
   return {
      closeTemplateSource() {
      },
      findTemplateSource( name ) {
         const filename = path.resolve( templateRootDirectory, name );
