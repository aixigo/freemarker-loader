...
const TEMPLATE_HASH_MODEL = 'freemarker.template.TemplateHashModel';
const TEMPLATE_METHOD_MODEL = 'freemarker.template.TemplateMethodModel';
...
function ObjectToJava( obj ) {
   ...
   Object.keys( obj ).forEach( key => {
      if (typeof obj[ key ] === 'function') {
         map.putSync( key, MethodToJava( obj, obj[ key ] ) );
      }
      else {
         map.putSync( key, toJava( obj[ key ] ) );
      }
   } );
   return map;
}
